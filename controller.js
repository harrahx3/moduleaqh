...
//	req.database.query('SELECT e.id as id, e.description as content, e.start as date, e.organisationid as author, e.title as title FROM BDECalendar AS e ORDER BY e.start DESC;', (error, result) => {
		req.database.query('SELECT e.id as id, e.description as content, e.start as date, e.organisationid as author, e.title as title, m.id_user as miduser FROM BDECalendar AS e JOIN core_membership AS m ON e.organisationid=m.id_group ORDER BY e.start DESC;', (error, result) => {
		...
		if (error) {
			...
			if (result.length > 0) {
				...
				/*req.database.query('SELECT * FROM core_membership AS m WHERE id_user=?;', [req.user.id], (errorm, resultm) => {

					if (errorm) {
						req.logger.error(errorm);
						res.sendStatus(500);
					} else {
						if (resultm.length > 0) {
							for (let i in resultm) {
								if (resultm[i]['id_group']==event.author) {
									event.
								}
							}
						}
